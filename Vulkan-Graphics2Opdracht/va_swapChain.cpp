...
namespace va {
	...
	void vaSwapChain::createRenderPass() {
		...
		depthAttachment.format = graphicsPipeline.findDepthFormat();
	...
	void vaSwapChain::createFramebuffers() {
		...
		for (size_t i = 0; i < _swapChainImageViews.size(); i++) {
			std::array<VkImageView, 3> attachments = {
				graphicsPipeline.colorImageView(),
				graphicsPipeline.depthImageView(),
	...
	void vaSwapChain::recreateSwapChain() {
		...
		createSwapChain();
		createImageViews();
		graphicsPipeline.createColorResources();
		graphicsPipeline.createDepthResources();
		createFramebuffers();
	}
